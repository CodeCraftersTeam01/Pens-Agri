...
class UserSessionService {
  ...
  // Default fallback session for Petani
  static const UserSession defaultPetaniSession = UserSession(
    userId: 1,
    name: 'Petani Gapura Barat',
    role: 'petani',
    desaId: 79,
    desaName: 'Gapura Barat',
    kecamatanName: 'Gapura',
    phone: '081234567890',
    nik: '3529000000000001',
    email: 'petani.gapura@sumenep.go.id',
  );
  ...
  static Future<UserSession> getSession() async {
    ...
    if (userId == null) {
      return defaultPetaniSession;
    }

    return UserSession(
      userId: userId,
      name: prefs.getString(_keyName) ?? defaultPetaniSession.name,
      role: prefs.getString(_keyRole) ?? 'petani',
      desaId: prefs.getInt(_keyDesaId) ?? defaultPetaniSession.desaId,
      desaName: prefs.getString(_keyDesaName) ?? defaultPetaniSession.desaName,
      kecamatanName: prefs.getString(_keyKecamatanName) ?? defaultPetaniSession.kecamatanName,
      phone: prefs.getString(_keyPhone) ?? defaultPetaniSession.phone,
      nik: prefs.getString(_keyNik) ?? defaultPetaniSession.nik,
      email: prefs.getString(_keyEmail) ?? defaultPetaniSession.email,
    );
  }
